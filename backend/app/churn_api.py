...
def dashboard_stats(db: Session = Depends(get_db)):
    ...
    recent_models = (
        db.query(TrainModel).order_by(TrainModel.train_date.desc()).limit(5).all()
    )
...
def train(
    background_tasks: BackgroundTasks, body: TrainRequest, db: Session = Depends(get_db)
):
    id_value = body.id
...
def list_upload_files(body: PageRequest, db: Session = Depends(get_db)):
    ...
    return {"page": page, "page_size": page_size, "total": total, "records": items}
...
def predict_model_background(predict_id: int, file_path: str, model_id: str):
    ...
    try:
        predict_record = (
            ...
            .filter(PredictionHistory.id == predict_id)
            .first()
        )
        ...
        if model:
            modeltrain.predict(
                ...
                output_path=predict_record.result1_path,
            )
...
def list_models(body: PageRequest, db: Session = Depends(get_db)):
    ...
    records = (
        db.query(TrainModel)
        .order_by(TrainModel.train_date.desc())
        .offset(offset)
        .limit(page_size)
        .all()
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit(1)
    uvicorn.run(
        app="churn_api:app", host="0.0.0.0", port=8000, log_level="info", reload=True
    )
